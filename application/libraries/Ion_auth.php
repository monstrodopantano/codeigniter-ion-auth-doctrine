...
class Ion_auth
{
	...
	public function __construct()
	{
        ...
        elseif( $this->config->item('use_doctrine', 'ion_auth') )
            $this->load->model('ion_auth_doctrine_model', 'ion_auth_model');
        else
		...
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			if ($user)
			{
                if( $this->config->item('use_doctrine', 'ion_auth') )
                    $identity = 'get'.ucfirst($this->config->item('identity', 'ion_auth'));

				$data = array(
					'identity' => $this->config->item('use_doctrine', 'ion_auth') ? $user->$identity() : $user->{$this->config->item('identity', 'ion_auth')},
					'forgotten_password_code' => $this->config->item('use_doctrine', 'ion_auth') ? $user->getForgottenPasswordCode() : $user->forgotten_password_code
				);
	...
	public function forgotten_password_check($code)
	{
		...
		else
		{
			if ($this->config->item('forgot_password_expiration', 'ion_auth') > 0) {
                ...
                if( $this->config->item('use_doctrine', 'ion_auth') ){
                    if (time() - $profile->getForgottenPasswordTime() > $expiration) {
                        //it has expired
	...
	public function in_group($check_group, $id=false, $check_all = false)
	{
		...
		else
		{
			...
			{
                if( $this->config->item('use_doctrine', 'ion_auth') )
                    $groups_array[$group->getGroups()->getId()] = $group->getGroups()->getName();
                else
